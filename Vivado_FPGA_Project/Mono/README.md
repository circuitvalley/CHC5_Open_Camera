# CHC5 Xilinx-IP Mono ISP - Vivado 2024.2 project

FPGA design for the CircuitValley **CHC5** camera board (Zynq-7020, `xc7z020clg400-2`).
It receives a 4-lane, 12-bit MIPI CSI-2 **mono** image sensor, processes the image with
...
```sh
cd CHC5_XILINX_MONO_FW
vivado -mode batch -source build.tcl
```

| Does | Does not |
|---|---|
| MIPI CSI-2 receive: 4 lanes, RAW12, D-PHY at 1250 Mb/s per lane | Demosaic (mono sensors only) |
| Crop window on the raw sensor image, right after the CSI-2 receiver | Colour correction / white balance |
| Black-level correction | Auto-exposure statistics |
| Gamma on the grey image | |
| Grey RGB (R = G = B) and YUV 4:2:2 conversion | |
| Pixel packing: raw mono, grey RGB, YUV, mono (8 formats) | |
...
- `CHC5_XILINX_MONO_FW/CHC5_XILINX_MONO_FW/`: the Vivado project, created by `rebuild.tcl`;
- `CHC5_XILINX_MONO_FW/output/`: the finished archive, written by `build.tcl`.
...
| **Digilent HDMI IP** | Included: `rgb2dvi` 1.4 and its TMDS interface, copied unmodified from Digilent's `vivado-library` (commit `f4613ff`) into `CHC5_XILINX_MONO_FW/third_party/`. Nothing to fetch. |
...
The script works from any directory (`vivado -mode batch -source <path>/CHC5_XILINX_MONO_FW/build.tcl`)
...
```
CHC5_XILINX_MONO_FW/output/chc5_xilinx_mono_fw.tar.xz
```
...
- **Create or open the project, build, and pack** from the GUI: **Tools -> Run Tcl Script...** ->
  `CHC5_XILINX_MONO_FW/build.tcl`. It does the same six steps. The GUI waits while the build runs.
- **Only create the project** (for example to edit the block design first): in the Tcl Console,
  `cd <path>/CHC5_XILINX_MONO_FW`, then **Tools -> Run Tcl Script...** -> `rebuild.tcl`. From a shell:
  `cd CHC5_XILINX_MONO_FW && vivado -source rebuild.tcl`.
- **Open an existing project:** `vivado CHC5_XILINX_MONO_FW/CHC5_XILINX_MONO_FW/CHC5_XILINX_MONO_FW.xpr`,
...
MIPI CSI-2 RX Subsystem (4 lanes, RAW12, 1250 Mb/s per lane)
  +-> Crop -> Black-level corrector --+-> Pixel packer 0 (raw mono)
                                      +-> Pixel packer 1 (raw mono)
                                      +-> Mono to gamma -> Gamma LUT -> Gamma to RGB (R = G = B) --+
                                                                                                   |
   +-----------------------------------------------------------------------------------------------+
   +-> Pixel packer 0 / 1 (grey RGB)
   +-> CSC RGB->YUV 4:2:2 -> YUV repack --+-> Pixel packer 0 / 1 (YUV, mono)
...
**Mono to gamma / Gamma to RGB** are two register-less adapters: the first puts the 12-bit mono
pixel (scaled to 8 bits) on one channel of the Xilinx gamma LUT, the second copies that channel
back out as grey RGB (R = G = B). They have no Linux driver and no device-tree node.
...
The two Xilinx HLS cores are reset through PS **EMIO GPIO**, because their Linux drivers
require a `reset-gpios` property and pulse it on every stream start. Both are active-low.
...
EMIO bits 0 and 2 are not used. These cores stay in reset while nothing is streaming.
Reading their registers then can hang the AXI bus, so only access them during streaming.
...
```sh
cd CHC5_XILINX_MONO_FW
rm -rf CHC5_XILINX_MONO_FW output .Xil
vivado -mode batch -source build.tcl
```
...
|---|---|
| `digilentinc.com:ip:rgb2dvi:1.4` not found, or the IP is locked | `third_party/digilent-vivado-library/` is missing or was moved. It must stay inside `CHC5_XILINX_MONO_FW/`. |
...
| Board part `circuitvalley.com:circuitvalley_chc5:part0:1.0` not found | `Circuitvalley_CHC5_Board/` is missing or not next to `CHC5_XILINX_MONO_FW/`. For your own projects, see [Installing the board files](#installing-the-board-files). |
...
| `build.tcl: build failed: synth_1 ..., impl_1 ...` | A run failed. Read `CHC5_XILINX_MONO_FW/CHC5_XILINX_MONO_FW.runs/synth_1/runme.log` or `impl_1/runme.log`. |
...
**Third-party code included:** `CHC5_XILINX_MONO_FW/third_party/digilent-vivado-library/` contains the
