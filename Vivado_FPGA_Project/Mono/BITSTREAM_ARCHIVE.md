...
The result is `CHC5_XILINX_MONO_FW/output/<manifest name>.tar.xz`, for example
`output/chc5_xilinx_mono_fw.tar.xz`. Upload it in the camera's web interface, in the bitstream
...
`CHC5_XILINX_MONO_FW/manifest.json` is the template `build.tcl` packs:
...
```json
{
  ...
  "manifest_version": 1,
  "manifest": {
    "name": "chc5_xilinx_mono_fw",
    "version": "1.0.0",
    "description": "Xilinx-IP mono ISP for 4-lane 12-bit MIPI CSI-2 mono sensors ...",
  ...
  "fpga": {
    ...
    "features": "crop,blkc,mono2gamma,gamma,gamma2rgb,csc,hdmi,camio,ptp,rgb888,xlnx_isp,xlnx_csi2rx",
    "max_res": "5568x3672",
    "bits": 12,
    "lanes": 4
  }
...
| `mono2gamma` | Mono-to-gamma adapter (mono fabric) | Informational; with no `demosaic` token the fabric counts as mono |
| `gamma` | Gamma stage | Informational |
| `gamma2rgb` | Gamma-to-grey-RGB adapter | Informational |
...
| `xlnx_isp` | Image pipeline built from Xilinx video IP | Informational |
...
run from `CHC5_XILINX_MONO_FW/` after a successful build:

<details>
<summary>Manual commands</summary>

```sh
IMPL=CHC5_XILINX_MONO_FW/CHC5_XILINX_MONO_FW.runs/impl_1
...
tar -C pkg --sort=name --mtime=@0 --owner=0 --group=0 --numeric-owner -cf chc5_xilinx_mono_fw.tar .
xz -T1 -e -z -f chc5_xilinx_mono_fw.tar                                # -> chc5_xilinx_mono_fw.tar.xz
tar -tvJf chc5_xilinx_mono_fw.tar.xz                                   # ./  ./bitstream.bin  ./manifest.json
```
