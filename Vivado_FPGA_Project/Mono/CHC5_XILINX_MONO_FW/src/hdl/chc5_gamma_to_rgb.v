/*
 * chc5_gamma_to_rgb.v - takes one gamma channel out as grey RGB (R = G = B)
...
module chc5_gamma_to_rgb #(
    parameter integer IN_COMP_WIDTH    = 8,
    ...
    input  wire [((PIXELS_PER_CLOCK*3*IN_COMP_WIDTH+7)/8)*8-1:0] s_axis_tdata,
    (* X_INTERFACE_INFO = "xilinx.com:interface:axis:1.0 S_AXIS TKEEP" *)
    input  wire [(PIXELS_PER_CLOCK*3*IN_COMP_WIDTH+7)/8-1:0]     s_axis_tkeep,
    ...
    localparam integer IW  = IN_COMP_WIDTH;
    ...
    generate
        for (k = 0; k < PPC; k = k + 1) begin : gen_pixel
            wire [IW-1:0]     sel  = s_axis_tdata[(k*3 + LANE)*IW +: IW];
            wire [REP*IW-1:0] rep  = {REP{sel}};
            wire [OW-1:0]     grey = rep[REP*IW-1 -: OW];

            for (c = 0; c < 3; c = c + 1) begin : gen_comp
                assign m_axis_tdata[(k*3 + c)*OW +: OW] = grey;
            end
        end
