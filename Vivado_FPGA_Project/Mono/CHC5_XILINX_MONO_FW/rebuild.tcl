# rebuild.tcl - recreates the CHC5_XILINX_MONO_FW Vivado project and block design
...
proc checkRequiredFiles { origin_dir} {
 ...
 "[file normalize "$origin_dir/src/hdl/chc5_mono_to_gamma.v"]"\
 "[file normalize "$origin_dir/src/hdl/chc5_gamma_to_rgb.v"]"\
...
set _xil_proj_name_ "CHC5_XILINX_MONO_FW"
...
set orig_proj_dir "[file normalize "$origin_dir/CHC5_XILINX_MONO_FW"]"
...
if { [get_files [list chc5_mono_to_gamma.v]] == "" } {
  import_files -quiet -fileset sources_1 ${::origin_dir}/src/hdl/chc5_mono_to_gamma.v
}
if { [get_files [list chc5_gamma_to_rgb.v]] == "" } {
  import_files -quiet -fileset sources_1 ${::origin_dir}/src/hdl/chc5_gamma_to_rgb.v
}
...
\
  ...
  set block_name chc5_mono_to_gamma
  set block_cell_name chc5_mono_to_gamma_0
  if { [catch {set chc5_mono_to_gamma_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $chc5_mono_to_gamma_0 eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
    set_property -dict [list \
    CONFIG.CHANNEL {1} \
    CONFIG.IN_PIXEL_WIDTH {12} \
    CONFIG.OUT_COMP_WIDTH {8} \
    CONFIG.PIXELS_PER_CLOCK {4} \
  ] $chc5_mono_to_gamma_0

  set block_name chc5_gamma_to_rgb
  set block_cell_name chc5_gamma_to_rgb_0
  if { [catch {set chc5_gamma_to_rgb_0 [create_bd_cell -type module -reference $block_name $block_cell_name] } errmsg] } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2095 -severity "ERROR" "Unable to add referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   } elseif { $chc5_gamma_to_rgb_0 eq "" } {
     catch {common::send_gid_msg -ssname BD::TCL -id 2096 -severity "ERROR" "Unable to referenced block <$block_name>. Please add the files for ${block_name}'s definition into the project."}
     return 1
   }
    set_property -dict [list \
    CONFIG.CHANNEL {1} \
    CONFIG.IN_COMP_WIDTH {8} \
    CONFIG.OUT_COMP_WIDTH {8} \
    CONFIG.PIXELS_PER_CLOCK {4} \
  ] $chc5_gamma_to_rgb_0
  ...
  connect_bd_intf_net -intf_net axis_broadcaster_1_M01_AXIS [get_bd_intf_pins axis_broadcaster_1/M01_AXIS] [get_bd_intf_pins chc5_mono_to_gamma_0/S_AXIS]
  ...
  connect_bd_intf_net -intf_net chc5_gamma_to_rgb_0_M_AXIS [get_bd_intf_pins chc5_gamma_to_rgb_0/M_AXIS] [get_bd_intf_pins axis_broadcaster_0/S_AXIS]
  connect_bd_intf_net -intf_net chc5_mono_to_gamma_0_M_AXIS [get_bd_intf_pins chc5_mono_to_gamma_0/M_AXIS] [get_bd_intf_pins v_gamma_lut_0/s_axis_video]
  ...
  connect_bd_intf_net -intf_net smartconnect_3_M02_AXI [get_bd_intf_pins smartconnect_3/M02_AXI] [get_bd_intf_pins v_proc_ss_1/s_axi_ctrl]
  ...
  connect_bd_intf_net -intf_net smartconnect_3_M04_AXI [get_bd_intf_pins smartconnect_3/M04_AXI] [get_bd_intf_pins chc5_axis_crop_0/s_axi]
  ...
  connect_bd_intf_net -intf_net v_gamma_lut_0_m_axis_video [get_bd_intf_pins v_gamma_lut_0/m_axis_video] [get_bd_intf_pins chc5_gamma_to_rgb_0/S_AXIS]
  ...
  [get_bd_pins chc5_mono_to_gamma_0/aclk] \
  [get_bd_pins chc5_gamma_to_rgb_0/aclk]
...
if { [get_property IS_LOCKED [ get_files -norecurse [list design_1.bd]] ] == 1  } {
  import_files -fileset sources_1 [file normalize "${origin_dir}/CHC5_XILINX_MONO_FW/CHC5_XILINX_MONO_FW.gen/sources_1/bd/design_1/hdl/design_1_wrapper.v" ]
} else {
