...
#define AT24_CHIP_DATA_PS(_name, _len, _flags, _page_size)		\
	static const struct at24_chip_data _name = {			\
		.byte_len = _len, .flags = _flags,			\
		.page_size = _page_size,				\
	}

#define AT24_CHIP_DATA_BS_PS(_name, _len, _flags,			\
			     _bank_addr_shift, _page_size)		\
	static const struct at24_chip_data _name = {			\
		.byte_len = _len, .flags = _flags,			\
		.bank_addr_shift = _bank_addr_shift,			\
		.page_size = _page_size,				\
...
AT24_CHIP_DATA_PS(at24_data_24c512,   524288  / 8, AT24_FLAG_ADDR16, 128);
AT24_CHIP_DATA_PS(at24_data_24c1024, 1048576  / 8, AT24_FLAG_ADDR16, 256);
AT24_CHIP_DATA_BS_PS(at24_data_24c1025, 1048576 / 8,
		     AT24_FLAG_ADDR16, 2, 128);
AT24_CHIP_DATA_PS(at24_data_24c2048, 2097152  / 8, AT24_FLAG_ADDR16, 256);
/* identical to 24c08 ? */
...
static int at24_probe(struct i2c_client *client)
{
	...
	if (err) {
		page_size = cdata->page_size ? cdata->page_size : 1;
	}
