...
#define XDMSC_PAD_STATS		(2)
#define XDMSC_NUM_PADS		(3)

struct xdmsc_dev {
	struct xvip_device xvip;
	struct media_pad pads[XDMSC_NUM_PADS];
	struct v4l2_mbus_framefmt formats[XDMSC_NUM_PADS];
	struct v4l2_mbus_framefmt default_formats[XDMSC_NUM_PADS];
			    ...
			    struct v4l2_subdev_format *fmt)
{
	...
	*__format = fmt->format;
	...
	if (fmt->pad == XVIP_PAD_SOURCE || fmt->pad == XDMSC_PAD_STATS) {
...
static int xdmsc_open(struct v4l2_subdev *subdev, struct v4l2_subdev_fh *fh)
{
	...
	*format = xdmsc->default_formats[XVIP_PAD_SOURCE];
	...
	format = v4l2_subdev_get_try_format(subdev, fh->state, XDMSC_PAD_STATS);
	*format = xdmsc->default_formats[XDMSC_PAD_STATS];
	return 0;
}
...
static int xdmsc_probe(struct platform_device *pdev)
{
	...
	*def_fmt = xdmsc->default_formats[XVIP_PAD_SINK];
	...
	def_fmt = &xdmsc->default_formats[XDMSC_PAD_STATS];
	*def_fmt = xdmsc->default_formats[XVIP_PAD_SOURCE];
	xdmsc->formats[XDMSC_PAD_STATS] = *def_fmt;

	xdmsc->pads[XVIP_PAD_SINK].flags = MEDIA_PAD_FL_SINK;
	xdmsc->pads[XVIP_PAD_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
	xdmsc->pads[XDMSC_PAD_STATS].flags = MEDIA_PAD_FL_SOURCE;

	/* Init Media Entity */
	subdev->entity.ops = &xdmsc_media_ops;
	rval = media_entity_pads_init(&subdev->entity, XDMSC_NUM_PADS,
				      xdmsc->pads);
	if (rval < 0)
		goto media_error;
