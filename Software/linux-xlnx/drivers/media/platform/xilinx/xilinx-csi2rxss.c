...
static void xcsi2rxss_program_active_lanes(struct xcsi2rxss_state *state)
{
	u32 pcr, max_lanes, lanes, readback;

	if (!state->enable_active_lanes)
		return;

	pcr       = xcsi2rxss_read(state, XCSI_PCR_OFFSET);
	max_lanes = FIELD_GET(XCSI_PCR_MAXLANES_MASK, pcr) + 1;
	lanes     = state->max_num_lanes;

	if (lanes < 1 || lanes > max_lanes) {
		dev_warn(state->dev,
			 "data-lanes = %u out of range for this bitstream (max %u), using %u\n",
			 lanes, max_lanes, max_lanes);
		lanes = max_lanes;
	}

	pcr &= ~XCSI_PCR_ACTLANES_MASK;
	pcr |= FIELD_PREP(XCSI_PCR_ACTLANES_MASK, lanes - 1);
	xcsi2rxss_write(state, XCSI_PCR_OFFSET, pcr);

	readback = FIELD_GET(XCSI_PCR_ACTLANES_MASK,
			     xcsi2rxss_read(state, XCSI_PCR_OFFSET)) + 1;
	if (readback != lanes)
		dev_err(state->dev,
			"active lanes: wrote %u, reads back %u -- this bitstream was NOT built with \"Enable Active Lanes\" despite xlnx,en-active-lanes in the DT\n",
			lanes, readback);
	else
		dev_dbg(state->dev, "active lanes: %u of %u\n",
			lanes, max_lanes);
}
...
static int xcsi2rxss_start_stream(struct xcsi2rxss_state *state)
{
	...
	xcsi2rxss_program_active_lanes(state);
...
static int xcsi2rxss_parse_of(struct xcsi2rxss_state *xcsi2rxss)
{
	...
	dev_info(dev, "vcx %s, %u data lanes (%s), data type 0x%02x\n",
