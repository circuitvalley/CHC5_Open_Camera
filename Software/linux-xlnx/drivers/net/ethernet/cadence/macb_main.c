...
static irqreturn_t macb_interrupt(int irq, void *dev_id)
{
	...
	while (status) {
		...
		if (unlikely(status & MACB_ZPTP_INTS)) {
			if (bp->caps & MACB_CAPS_ISR_CLEAR_ON_WRITE)
				queue_writel(queue, ISR, status & MACB_ZPTP_INTS);
			if (macb_zynq_ptp(bp))
				zynq_gem_ptp_irq(bp, status);
		}
#endif
...
#ifdef CONFIG_MACB_ZYNQ_PTP
	if (unlikely(skb_shinfo(skb)->tx_flags & SKBTX_HW_TSTAMP) &&
	    macb_zynq_ptp(bp) && bp->tstamp_config.tx_type == HWTSTAMP_TX_ON)
		zynq_gem_ptp_txqueue(bp, skb);
#endif
...
{
	return dev->netdev_ops == &macb_netdev_ops;
}
#endif
...
#ifdef CONFIG_MACB_ZYNQ_PTP
		if (!gem_has_ptp(bp) && (bp->caps & MACB_CAPS_GEM_PTP_EVT_REGS) &&
		    GEM_BFEXT(TSU, gem_readl(bp, DCFG5))) {
			bp->ptp_info = &zynq_gem_ptp_info;
			zynq_gem_ptp_setup(bp);
		}
#endif
	}
...
static const struct macb_config zynq_config = {
	.caps = MACB_CAPS_GIGABIT_MODE_AVAILABLE | MACB_CAPS_NO_GIGABIT_HALF |
		MACB_CAPS_NEEDS_RSTONUBR | MACB_CAPS_GEM_PTP_EVT_REGS,
	.dma_burst_length = 16,
	.clk_init = macb_clk_init,
	.init = macb_init,
	.usrio = &macb_default_usrio,
...
static struct platform_driver macb_driver = {
	...
	.driver		= {
		...
		.pm	= &macb_pm_ops,
#ifdef CONFIG_MACB_ZYNQ_PTP
		.dev_groups	= macb_zynq_ptp_groups,
#endif
	},
};
