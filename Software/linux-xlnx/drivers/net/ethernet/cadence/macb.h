...
#define MACB_CAPS_GEM_PTP_EVT_REGS		0x00002000
...
#ifdef CONFIG_MACB_ZYNQ_PTP
#define MACB_ZPTP_RX_EV_INTS	(MACB_BIT(SFR) | MACB_BIT(DRQFR))
#define MACB_ZPTP_TX_EV_INTS	(MACB_BIT(SFT) | MACB_BIT(DRQFT))
#define MACB_ZPTP_RX_PEER_INTS	(MACB_BIT(PDRQFR) | MACB_BIT(PDRSFR))
#define MACB_ZPTP_TX_PEER_INTS	(MACB_BIT(PDRQFT) | MACB_BIT(PDRSFT))
#define MACB_ZPTP_RX_INTS	(MACB_ZPTP_RX_EV_INTS | MACB_ZPTP_RX_PEER_INTS)
#define MACB_ZPTP_TX_INTS	(MACB_ZPTP_TX_EV_INTS | MACB_ZPTP_TX_PEER_INTS)
#define MACB_ZPTP_INTS		(MACB_ZPTP_RX_INTS | MACB_ZPTP_TX_INTS)

enum { ZPTP_EV, ZPTP_PEER, ZPTP_NGROUPS };
...
struct macb_zptp_stats {
	u32 rx_irqs[ZPTP_NGROUPS];
	u32 rx_stamped;
	u32 rx_unmatched;
	u32 tx_irqs[ZPTP_NGROUPS];
	u32 tx_stamped;
	u32 tx_timeout;
	u32 tx_busy;
	u32 tx_orphan;
...
#ifdef CONFIG_MACB_ZYNQ_PTP
	struct cyclecounter zptp_cc;
	struct timecounter zptp_tc;
	u32 zptp_mult;
	spinlock_t zptp_ts_lock;
	struct macb_zptp_rx zptp_rx[ZPTP_NGROUPS];
	struct macb_zptp_tx zptp_tx[ZPTP_NGROUPS];
	struct macb_zptp_stats zptp_stats;
	struct delayed_work zptp_tx_work;
	u32 zptp_ints;
	bool zptp_running;
	struct macb_zptp_follow zptp_follow;
#endif
...
#ifdef CONFIG_MACB_ZYNQ_PTP
extern struct macb_ptp_info zynq_gem_ptp_info;
extern const struct attribute_group *macb_zynq_ptp_groups[];
bool macb_is_macb_netdev(const struct net_device *dev);
...
static inline bool macb_zynq_ptp(struct macb *bp)
{
	return bp->ptp_info == &zynq_gem_ptp_info;
}
#else
static inline bool macb_zynq_ptp(struct macb *bp) { return false; }
#endif
