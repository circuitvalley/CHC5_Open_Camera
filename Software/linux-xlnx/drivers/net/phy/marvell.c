...
#if IS_ENABLED(CONFIG_MARVELL_PHY_PTP)
	if (marvell_phy_ptp_event_active(phydev))
		def_config = (def_config & ~0x00f0) |
			     (MII_88E1318S_PHY_LED_FUNC_HI_Z << 4);
#endif
...
#if IS_ENABLED(CONFIG_MARVELL_PHY_PTP)
	err = phy_sfp_probe(phydev, &m88e1510_sfp_ops);
	if (err)
		return err;

	err = devm_marvell_phy_ptp_probe(phydev);
	if (err)
		phydev_warn(phydev, "PTP timestamping unavailable: %pe\n",
			    ERR_PTR(err));
